...
const saveOrUpdateUserPrompt = async (userId, channelTheme, genderText) => {
    if (!channelTheme || !genderText) {
        throw new Error("Missing channelTheme or genderText");
    }
    ...
    const prompt = await UserPrompt.findOneAndUpdate(
        { userId },
        { channelTheme, genderText, generalPrompt },
