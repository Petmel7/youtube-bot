...
const { saveOrUpdateUserPrompt, getUserPromptData, updateUserGenderService } = require("../services/userPromptService");
...
const handleUserPrompt = async (req, res) => {
    try {
        ...
        const prompt = await saveOrUpdateUserPrompt(userId, channelTheme, genderText);
        res.json({ success: true, prompt });
    } catch (error) {
